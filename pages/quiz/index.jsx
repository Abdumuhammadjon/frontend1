...
export default function Home() {
  ...
  useEffect(() => {
    const fetchSubjects = async () => {
      setLoading(true);
      try {
        const response = await axios.get("https://60maktab.onrender.com/api/subjects");
  ...
  const fetchQuestions = async (subjectId) => {
    ...
    try {
      const response = await axios.get(
        `https://60maktab.onrender.com/api/subject/${subjectId}`
      );
  ...
  const handleSaveAnswers = async () => {
    ...
    try {
      const res = await axios.post("https://60maktab.onrender.com/api/save-answers", {
        answers,
        userId,
